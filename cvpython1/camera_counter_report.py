...
def load_yolov5_model():
    try:
        # this will download the model if not present (internet needed first time)
        model = torch.hub.load("ultralytics/yolov5", "yolov5s", pretrained=True)
        print("Loaded YOLOv5s via torch.hub")
        return model
    except Exception as e:
        print("torch.hub load failed:", e)
        # try ultralytics package (YOLOv8) if installed
        try:
...
class FaceCropper:
    def __init__(self):
        # try OpenCV's DNN face detector model (if available in opencv installation)
        self.face_net = None
        try:
            # default opencv comes with a deep learning face detector? handle gracefully
            # We'll fallback to Haarcascade if dnn not available
